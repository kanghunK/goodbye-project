...
const Editor = ({ onSubmit, formRef }) => {
	...
	return (
		...
		>
			...
			<Notice>
				추후에 추모글 수정, 삭제에 필요합니다. 원하는 비밀번호로
				등록하세요
			</Notice>
			<Form.Item
				name={'password'}
...
const remembrance = () => {
		...
		try {
			comments.forEach(async comment => {
				const res = await axios.get(
					`/api/remembrances/${remembranceId}/comments/${comment._id}`,
				);
				const data = {
	...
	const getRemembranceData = async remembranceId => {
		try {
			...
			// eslint-disable-next-line no-shadow
			const { _id, photo, comments, fullName, dateOfBirth, dateOfDeath } =
				res.data;
			console.log(res);
			setUserData({
				fullName,
			...
			<AppLayout>
				...
				<CommentTree>
					...
					<Button
						...
						onClick={() => setCommentArea(prev => !prev)}
					>
						{visibleCommentArea
							? '댓글 양식 가리기'
							: '댓글 작성하기'}
